...
function connect(bin, options, callback) {
  ...
  if (options.readyFileId) {
    ...
    readyFileName = readyFileName + "_" + generateUUID();
  }
...
function downloadWithRetry(options, callback) {
  tryRun(0, {
    logger: options.logger,
...
function downloadAndRun(options, callback) {
  ...
  async.waterfall([
    async.apply(downloadWithRetry, options),
    function (bin, next) {
...
module.exports.download = downloadWithRetry;
